    ...
    <el-container id="sendOrder">
        ...
        <el-main style="padding:10px;overflow-y:hidden;height:100%;">
            <el-table :data="orderList" style="width: 100%;" height="49%" ref="orderListTable" @current-change="getOrderDetail" highlight-current-row>
                ...
                <el-table-column prop="create_time" label="订单状态" width="200">
                    <template slot-scope="scope">
                        <span v-if="scope.row.type==1 && scope.row.pick == 0" :style="scope.row.pay==0?'color:#ccc;':'color:#E04C4C'">待提货(提货)</span>
                        ...
                        <span><i v-if="scope.row.type==1" style="font-size:16px;color:#DFB31F;" class="icon iconfont icon-icontihuo"></i>
                        <i v-else-if="scope.row.type==2" style="font-size:16px;color:#17C295;" class="icon iconfont icon-che-tianchong"></i></span>
                        <!-- <span style="margin-left:6px;display:inline-block;" v-if="scope.row.type==2 && scope.row.dispatch == 0"><el-button @click="sendOrder(scope.row)" size="mini">配送</el-button></span> -->
                    </template>
                </el-table-column>
                <el-table-column prop="create_time" label="预计到货时间" width="172">
                    <template slot-scope="scope">
                        <span>{{scope.row.ps_date}} {{scope.row.ps_time}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="dispatch_time" label="配送完成时间" width="172">
                    <template slot-scope="scope">
                        <span>{{scope.row.dispatch_time||'无'}}</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="express_fee" label="运费">
                    <template slot-scope="scope">
                        <span>{{scope.row.express_fee||'无'}}</span>
                    </template>
                </el-table-column>
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        data(){
            return {
                orderStatusList:[{name:'待配送',id:'3'},{name:'配送中',id:'4'},{name:'配送完成',id:'10'},{name:'全部',id:'-1'}],
                form:{
        ...
        methods:{
            reload(){
                ...
                let initTime = [this.formatTime(new Date(today.getTime() - 3600 * 1000 * 24 * 0)),this.formatTime(new Date(today.getTime() + 3600 * 1000 * 24 * 0))]
            ...
            // 配送（不要了）
            sendOrder(){
                this.$confirm(`是否确认配送？`, '提示', 
                    {confirmButtonText: '是',   cancelButtonText: '否',type: 'warning'
                }).then(() => {
                    let param = {
                        user_token:this.$store.state.loginData.user_token,
                        access_token: this.$store.state.loginData.access_token,
                        action:"goDistribution",
                    };
                    let goods = [];
                    if(this.orderDetail_goods.length>0){
                        for(let i = 0;i<this.orderDetail_goods.length;i++){
                            let item = {
                                order_id:this.orderDetail_goods[i].order_id,
                                goods_id:this.orderDetail_goods[i].goods_id,
                                key_id:this.orderDetail_goods[i].id,
                            }
                            goods.push(item)
                        }
                    }
            ...
            getOrderDetail(val){
                ...
                this.$http.post(`${this.$store.state.host}${this.$store.state.config.vip}`, param).then((res) => {
                    if(res.data.code = 1){
                        ...
                        // 小票会员信息
                        this.printData.userData.sh_address = val.address||null;
                        this.printData.userData.sh_name = val.sh_name||null;
                        this.printData.userData.sh_mobile=val.sh_mobile||null;
